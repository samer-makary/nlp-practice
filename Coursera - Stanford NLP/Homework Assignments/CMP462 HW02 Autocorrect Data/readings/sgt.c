...

{
    ...
    while (fgets(line, MAX_LINE, stdin) != NULL && rowNumber < MAX_ROWS)
    {
        ...
        if (*ptr == '\0')
            continue;
        if ((integer = strtok(ptr, whiteSpace)) == NULL || (i = atoi(integer)) < 1)
        {
        ...
        {
            fprintf(stderr, "Frequency not in ascending order, line %d\n", lineNumber);
            return(error);
        }
        r[rowNumber] = i;
        if ((integer = strtok(NULL, whiteSpace)) == NULL || (i = atoi(integer)) < 1)
        {
